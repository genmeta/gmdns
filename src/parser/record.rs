...
pub fn be_record(input: &[u8]) -> nom::IResult<&[u8], ResourceRecord<'_>> {
    ...
    let (remain, rdata) = be_rdata(remain, Type::from_u16(typ), rdlen)?;
...
fn be_rdata(input: &[u8], typ: Type, rdlen: u16) -> nom::IResult<&[u8], RData> {
    match typ {
        ...
        Type::CNAME => be_name(input).map(|(remian, name)| (remian, RData::Cname(name))),
        Type::TXT => map(take(rdlen), RData::Txt).parse(input),
        Type::SRV => map(take(rdlen), RData::Srv).parse(input),
...
impl<T: BufMut> WriteRecord for T {
    fn put_record(&mut self, record: &ResourceRecord) {
        ...
        match &record.data {
            ...
            RData::Srv(srv) => self.put_slice(srv),
